...
class TestDoorKey(unittest.TestCase):
    ...
    def test_transform_action(self):
        ...
        # and agent should change his state, otherwise performing unallowed action state should be the same
        new_state, reward, is_episode_end = self.env.step(4)
        self.assertNotEqual(state.tolist(), new_state.tolist())

        state = self.env.reset()
        new_state, reward, is_episode_end = self.env.step(6)  # Unallowed action
    ...
    def test_observe(self):
        ...
        with patch.object(self.env._base_env, 'step', return_value=mocked_step_return): 
            new_state, reward, is_episode_end = self.env.step(0)
