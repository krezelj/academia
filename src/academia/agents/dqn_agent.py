...
class DQNAgent(Agent):
    ...
    def __build_network_(self):
        ...
        Description:
             The method creates instances of the neural network specified by `nn_architecture` and initializes the optimizer with the Adam optimizer. It also initializes the target network with the same architecture and loads its initial weights from the main network. The target network is set to evaluation mode during training.

        Notes:
            - The neural networks are moved to the appropriate device (CPU or CUDA) using the `to(device)` method.
            - The target network is initialized with the same architecture as the main network and its weights are loaded from the main network using `load_state_dict`.
            - The target network is set to evaluation mode (`eval()`) to ensure consistent target Q-value computations during training.
    ...
    def remember(self, state, action, reward, next_state, done):
        ...
        Description:
            The `remember` function appends the experience tuple `(state, action, reward, next_state, done)` to the replay memory of the DQNAgent. The replay memory is a data structure used to store and sample past experiences, allowing the agent to learn from previous interactions with the environment. If the replay memory exceeds its maximum size (`REPLAY_MEMORY_SIZE`), the function removes the oldest experience to make room for the new one.

        Notes:
            - `state` and `next_state` should be representations of the agent's observations in the environment.
            - `action` represents the action taken by the agent based on the current state.
            - `reward` is the numerical reward received by the agent after taking the specified action.
            - `done` is a boolean flag indicating whether the episode has terminated or truncated (`True`) or not (`False`).
    ...
    def get_action(self, state, legal_mask=None, greedy=False) -> int:
        ...
        Parameters:
            - `state (array-like)`: The current state representation used to make the action selection decision.
            - `legal_mask (array-like, optional)`: A binary mask indicating the legality of actions. If provided, restricts the agent's choices to legal actions.
            - `greedy (bool, optional)`: A boolean flag indicating whether to force a greedy action selection. If True, the function always chooses the action with the highest Q-value, ignoring exploration.

        Returns:
            - `int`: The index of the selected action.

        Description:
            The `get_action` function selects an action for the agent based on the current state using the epsilon-greedy strategy. If the `legal_mask` is provided, it restricts the agent's choices to legal actions, ensuring that illegal actions are not considered. The `greedy` flag allows forcing a purely exploitative behavior, where the action with the highest Q-value is always chosen, ignoring exploration. If `greedy` is False or the exploration condition is met (based on epsilon value), the function chooses a random action with a probability of epsilon or explores other legal actions based on the provided `legal_mask`. If no legal mask is provided, the function considers all possible actions.

        Notes:
            - Ensure that the `state` input is compatible with the input size expected by the agent's neural network.
            - If the `legal_mask` is not provided, all actions are considered during action selection.
    ...
    def update_target(self):
        ...
        Description:
            Updates the target network's weights with the main network's weights. This function synchronizes the parameters of the target network to match those of the main network, ensuring consistency between the two networks during the training process. It is called periodically to stabilize the training of the DQNAgent.

        Note:
            It is essential to call this function periodically, especially after a certain number of training steps, to ensure that the target network's weights are in line with the main network. Keeping the target network updated is crucial for stable and effective training of the DQN agent.
    ...
    def update(self, state, action, reward: float, new_state, is_terminal: bool):
        ...
        Description:
            The `update` function is responsible for updating the DQN agent's Q-values based on the provided experience tuple. It adds the experience to the replay memory and performs a mini-batch update if the replay memory size reaches the specified batch size. The agent uses the Mean Squared Error (MSE) loss between predicted Q-values and target Q-values to update its neural network weights.

            If the `is_terminal` flag is `True`, the function increments the update counter and checks if it's time to update the target network weights. The target network is updated periodically to stabilize the learning process.

        Notes:
            - Ensure that the DQN agent has been properly initialized with the necessary parameters before calling this function.
            - The function performs an update only if the replay memory size reaches the specified batch size (`batch_size`).
            - The target network is updated periodically based on the `UPDATE_TARGET_FREQ` parameter.
        """
    ...
    def replay(self) -> (torch.Tensor, torch.Tensor):
        """
        This function samples a mini-batch from the replay memory and prepares states and corresponding target Q-values for optimization.

        Returns:
            - `states (torch.Tensor)`: Tensor containing the states sampled from the replay memory.
                Shape: (batch_size, state_features)
            - `targets (torch.Tensor)`: Tensor containing the corresponding target Q-values for the sampled states and actions.
                Shape: (batch_size, num_actions)

        Description:
            The `replay` function is responsible for generating a mini-batch of experiences from the replay memory and computing target Q-values for each state-action pair. For each sampled experience tuple (state, action, reward, next_state, done), it calculates the target Q-value as follows:
                - If the episode is not done, the target Q-value is computed as the sum of the immediate reward and the discounted maximum Q-value of the next state according to the target network.
                - If the episode is done, the target Q-value is set equal to the immediate reward.

            After computing the target Q-values, the function constructs tensors of states and target Q-values, both of which are moved to the appropriate device (CPU or CUDA) for optimization. The states tensor represents the sampled states, and the targets tensor contains the corresponding target Q-values.

        Notes:
            - Ensure that the `replay` function is called when the replay memory contains enough experiences to form a mini-batch (i.e., when `len(self.memory) >= self.batch_size`).
            - The function constructs tensors from the sampled states and target Q-values, which can be directly used for training the neural network.
    ...
    def save(self, path: str):
        ...
        Notes:
            - Ensure that the file path provided in `save_path` is writable and has appropriate permissions.
    ...
    def load(cls, path: str) -> 'DQNAgent':
        ...
        Description:
            The `load` method allows loading a pre-trained neural network model's state dictionary from a specified file path. This method is particularly useful when you want to continue training from a pre-trained model or use a pre-trained model for making predictions in a real-time environment. After loading the model, it is set to evaluation mode, ensuring that gradients are not computed during inference, which reduces memory consumption and computation time.

        Notes:
            - Ensure that the `model_path` parameter points to a valid saved model file with the appropriate model architecture and state dictionary.
